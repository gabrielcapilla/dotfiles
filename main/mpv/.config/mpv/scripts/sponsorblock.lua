...
local options = {
    ...
    skip_categories = "sponsor,outro,filler,selfpromo",
...
function getranges(_, exists, db, more)
    ...
    local sponsors
    local args = {options.python_path, sponsorblock, "ranges", db, options.server_address, youtube_id,
                  options.categories, tostring(options.sha256_length)}
    if not legacy then
...
function skip_ads(name, pos)
    ...
    for uuid, t in pairs(ranges) do
        if (options.fast_forward == uuid or not options.skip_once or not t.skipped) and t.start_time <= pos and
            t.end_time > pos then
            ...
            if options.report_views or options.auto_upvote then
                local args = {options.python_path, sponsorblock, "stats", database_file, options.server_address,
                              youtube_id, uuid, options.report_views and "1" or "", uid_path, options.user_id,
...
function vote(dir)
    ...
    last_skip.dir = dir
    local args = {options.python_path, sponsorblock, "stats", database_file, options.server_address, youtube_id,
                  last_skip.uuid, "", uid_path, options.user_id, dir}
    if not legacy then
        mp.command_native_async({
            name = "subprocess",
            playback_only = false,
...
function update()
    mp.command_native_async({
        name = "subprocess",
        playback_only = false,
        args = {options.python_path, sponsorblock, "update", database_file, options.server_address}
    }, getranges)
end
...
function file_loaded()
    ...
    local urls = {"ytdl://([%w-_]+).*", "https?://youtu%.be/([%w-_]+).*",
                  "https?://w?w?w?%.?youtube%.com/v/([%w-_]+).*", "/watch.*[?&]v=([%w-_]+).*", "/embed/([%w-_]+).*"}
    youtube_id = nil
    ...
    if options.display_name ~= "" then
        local args = {options.python_path, sponsorblock, "username", database_file, options.server_address, youtube_id,
                      "", "", uid_path, options.user_id, options.display_name}
        if not legacy then
            mp.command_native_async({
                name = "subprocess",
                playback_only = false,
...
function submit_segment(category)
    ...
    elseif segment.progress <= 1 then
        ...
        local submit
        local args = {options.python_path, sponsorblock, "submit", database_file, options.server_address, youtube_id,
                      tostring(start_time), tostring(end_time), uid_path, options.user_id, category or "sponsor"}
        if not legacy then
