...
local user_opts = {
    ...
    language = 'spa',
    'eng', -- eng=English, spa=Spanish, chs=Chinese
...
local language = {
    ...
    ['spa'] = {
        welcome = '{\\fs24\\1c&H0&\\1c&HFFFFFF&}Arrastra archivos o enlaces aquí', -- this text appears when mpv starts
        off = 'OFF',
        na = 'n/d',
        none = 'ninguno',
        video = 'Video',
...
function get_slider_ele_pos_for(element, val)
    ...
    local ele_pos = scale_value(element.slider.min.value, element.slider.max.value, element.slider.min.ele_pos,
...
function get_slider_value_at(element, glob_pos)
    ...
    local val = scale_value(element.slider.min.glob_pos, element.slider.max.glob_pos, element.slider.min.value,
        element.slider.max.value, glob_pos)
...
function add_area(name, x1, y1, x2, y2)
    ...
    table.insert(osc_param.areas[name], {
        x1 = x1,
        y1 = y1,
        x2 = x2,
        y2 = y2
...
function prepare_elements()
    ...
    for _, element in pairs(elements) do
        ...
        if (element.type == 'box') then
            ...
            ass_draw_rr_h_cw(static_ass, 0, 0, elem_geo.w, elem_geo.h, element.layout.box.radius,
                    ...
                    if not thumbfast.disabled then
                        ...
                        if osd_w then
                            ...
                            mp.commandv("script-message-to", "thumbfast", "thumb",
                                mp.get_property_number("duration", 0) * (sliderpos / 100), thumbX, thumbY)
...
function osc_init()
    ...
    if user_opts.showjump then
        ...
        ne.eventresponder['enter'] = -- function () mp.command('seek -5') end
        function()
            mp.commandv('seek', -jumpamount, jumpmode)
        end
    ...
    if not (state.anitype == nil) then
        ...
        if (now < state.anistart + (user_opts.fadeduration / 1000)) then
            ...
            if (state.anitype == 'in') then -- fade in
                osc_visible(true)
                state.animation = scale_value(state.anistart, (state.anistart + (user_opts.fadeduration / 1000)), 255,
                    0, now)
            elseif (state.anitype == 'out') then -- fade out
                state.animation = scale_value(state.anistart, (state.anistart + (user_opts.fadeduration / 1000)), 0,
                    255, now)
            end
...
function process_event(source, what)
    ...
    elseif source == 'mouse_move' then
        ...
        local mouseX, mouseY = get_virt_mouse_pos()
        if (user_opts.minmousemove == 0) or (not ((state.last_mouseX == nil) or (state.last_mouseY == nil)) and
...
mp.set_key_bindings({{'mouse_move', function(e)
    process_event('mouse_move', nil)
end}, {'mouse_leave', mouse_leave}}, 'showhide', 'force')
mp.set_key_bindings({{'mouse_move', function(e)
    process_event('mouse_move', nil)
end}, {'mouse_leave', mouse_leave}}, 'showhide_wc', 'force')
do_enable_keybindings()

-- mouse input bindings
mp.set_key_bindings({{"mbtn_left", function(e)
    process_event("mbtn_left", "up")
end, function(e)
    process_event("mbtn_left", "down")
end}, {"shift+mbtn_left", function(e)
    process_event("shift+mbtn_left", "up")
end, function(e)
    process_event("shift+mbtn_left", "down")
end}, {"mbtn_right", function(e)
    process_event("mbtn_right", "up")
end, function(e)
    process_event("mbtn_right", "down")
end}, -- alias to shift_mbtn_left for single-handed mouse use
{"mbtn_mid", function(e)
    process_event("shift+mbtn_left", "up")
end, function(e)
    process_event("shift+mbtn_left", "down")
end}, {"wheel_up", function(e)
    process_event("wheel_up", "press")
end}, {"wheel_down", function(e)
    process_event("wheel_down", "press")
end}, {"mbtn_left_dbl", "ignore"}, {"shift+mbtn_left_dbl", "ignore"}, {"mbtn_right_dbl", "ignore"}}, "input", "force")
mp.enable_key_bindings('input')
